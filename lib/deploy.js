  ...
  return connect(options).then(function(conn) {
    ...
    DEPLOY_OPTIONS.forEach(function(prop) {
      if (typeof options[prop] !== 'undefined') {
        deployOpts[prop] = options[prop];
      }
    });
    return conn.metadata
      .deploy(zipStream, deployOpts)
 ...
 *
 */
function deployFromDirectory_bak(packageDirectoryPath, options) {
...
function reportDeployResult(res, logger, verbose) {
  var message = res.success
    ? 'Deploy Succeeded' +
      (res.status === 'SucceededPartial' ? ' Partially.' : '.')
    : res.done
      ? 'Deploy Failed.'
  ...
  if (details) {
    ...
    if (testResult && Number(testResult.numTestsRun) > 0) {
      ...
      testFailures.forEach(function(f) {
        logger.log(
          ' - ' +
            (typeof f.namespace === 'string' ? f.namespace + '__' : '') +
            f.name +
            '.' +
            f.methodName
        );
      ...
      if (verbose) {
        ...
        codeCoverages.forEach(function(s) {
          var coverage = Math.floor(
            100 - 100 * (s.numLocationsNotCovered / s.numLocations)
          );
          if (isNaN(coverage)) {
            coverage = 100;
          }
          logger.log(
            ' - ' +
              ...
              coverage +
              ' %] ' +
              (typeof s.namespace === 'string' ? s.namespace + '__' : '') +
              s.name
          );
        });
      }
